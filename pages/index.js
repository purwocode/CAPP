...
export default function LoginPage() {
  ...
  // validasi tombol
  const isDisabled = useEmail ? email.trim()==="" : phone.trim().length<14;

  // submit → munculkan popup
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!isDisabled) setShowPopup(true);
  };
              ...
              <>
                ...
                <div className="phoneField">
                  <button type="button" className="country">
                    <span className="flag" aria-hidden>  {/* US flag SVG kept as-is */}
